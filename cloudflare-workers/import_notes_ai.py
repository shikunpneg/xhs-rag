...
使用Workers AI API生成嵌入（与检索时使用相同模型）
"""
    ...
    for i in range(0, len(notes), batch_size):
        ...
        try:
            ...
            for note in batch:
                ...
                embedding = generate_embedding_via_api(text_to_embed)
                if not embedding:
                    print(f'  ⚠️ 跳过: {note["title"][:30]}... (嵌入生成失败)')
                    failed += 1
                    errors.append({'note_id': note['note_id'], 'error': '嵌入生成失败'})
                    continue
            ...
            if notes_with_embeddings:
                ...
                if response.status_code == 200:
                    ...
                    if result.get('successful'):
                        ...
                        print(f'  ✅ 成功添加 {result["successful"]}/{len(notes_with_embeddings)} 条')
                    ...
                    failed += len(notes_with_embeddings)
